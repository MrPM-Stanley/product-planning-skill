...
**深層需求**（EFT / Psychodynamic）
- 表層動機：[使用者說出口的理由]
- 底下的情緒：[沒說出口但真正驅動行為的情緒，例如羞恥、補償、渴望被認同]
- 重要性：很多使用者自己也說不清楚真正的驅動力。產品如果只解決表層問題，留存率會差——因為真正的癢沒被搔到。

**核心價值觀**（ACT）
- [2-3 個關鍵字，例如：自主、專業、被尊重]
- 「想成為的那個人」：[一句話描述]
- 重要性：價值觀決定使用者「願意為什麼付代價」。功能解決問題，價值觀創造忠誠。
```
...
> - **Feeling（感受）**（Barrett 情緒粒度）：至少用兩個具體情緒詞 + 強度（1-5）。不要寫「焦慮」，要寫「怕被同事看穿的社交焦慮（4）+ 不確定自己有沒有進步的模糊焦慮（3）」。不同種類的情緒對應不同的產品解法。
...
```
| 編號 | 痛點描述 | 痛點類型 | 來源 Persona | 出現在哪個階段 | 影響程度（高/中/低） | 出現頻率（高/中/低） |
|---|---|---|---|---|---|---|
| P1 | | [功能性/情緒性/意義性] | | | | |
| P2 | | [功能性/情緒性/意義性] | | | | |
```

**痛點類型說明**（三種類型來自不同心理學派，對應不同的解法風格）：
- **功能性**（CBT）：做不到、不會用、流程卡住、效率低——使用者知道問題是什麼，需要「修正」
- **情緒性**（EFT）：害怕、羞恥、挫敗、不被理解、孤獨——使用者可能說不清楚，但感覺不舒服
- **意義性**（ACT）：不知道為什麼要做、覺得沒意義、跟自己的價值觀衝突——使用者可能已經有工具但還是不滿足

**自我檢查**：如果所有痛點都是功能性的，很可能遺漏了情緒層和意義層——回頭檢查 Persona 卡片的「深層需求」和「核心價值觀」欄位。
...
```
| HMW 問題 | 解決型解法（CBT） | 接納型解法（ACT） | 情感表達型解法（EFT） |
|---|---|---|---|
| [HMW1] | | | |
| [HMW2] | | | |
```

**三種解法風格說明**（強制從不同角度發想，避免所有解法都是同一種思維）：
- **解決型**（CBT）：修正錯誤認知或行為，讓問題消失。例：推播提醒、進度儀表板、教學引導
- **接納型**（ACT）：問題不消失，但連結到價值觀，讓使用者帶著問題前進。例：「你想成為怎樣的人」目標設定、價值觀儀式、meaning-making 機制
- **情感表達型**（EFT）：讓被壓抑的情緒有安全的出口，光是被理解就是解法。例：不評分的樹洞模式、同儕分享社群、「你不是一個人」的情感連結

不是每個 HMW 都需要三種解法——有些問題天然偏功能性（適合 CBT），有些偏情緒性（適合 EFT）。但強制思考三種風格可以避免盲點。如果某種風格真的不適用，寫「不適用，因為...」並說明原因。
...
在切入點詳細分析後面，標註這個切入點主要靠哪種心理學驅動：

- **認知修正（CBT）**：讓使用者「想通」——產品核心價值在於幫人修正錯誤認知或行為
- **價值連結（ACT）**：讓使用者「想成為那個人」——產品核心價值在於連結到使用者的價值觀和身份認同
- **情感釋放（EFT）**：讓使用者「被理解」——產品核心價值在於提供情感出口或同理連結
- **社會比較（Psychodynamic）**：讓使用者「不輸別人」——產品核心價值在於地位、歸屬、認同

這個分類很重要，因為它會直接影響後續的行銷語氣和廣告策略方向（與 ad-planning 銜接）。一個「CBT 驅動型」產品的廣告通常走 PAS 文案結構，一個「ACT 驅動型」產品的廣告則走 Values-Action 結構。
...
- 痛點區塊用紅色左邊框標記
- 「深層需求」和「核心價值觀」區塊用紫色左邊框標記，視覺上區分心理學新增欄位

**User Journey Map 分段詳述：**
- 用手風琴（accordion）呈現，預設收合
- 點擊展開該階段的 Doing / Thinking / Feeling / Stakeholder / Problem
- Feeling 欄位要呈現 Barrett 情緒粒度（至少兩個具體情緒詞 + 強度）
- 展開動畫要平滑

**痛點彙整表：**
- 痛點類型用色標標籤呈現：功能性（藍色）、情緒性（紅色）、意義性（紫色）
- 如果所有痛點都是同一類型，在表格下方顯示警示提醒

**解法發想表：**
- 三欄分別用不同底色：解決型（藍色淡底）、接納型（綠色淡底）、情感表達型（橘色淡底）
- 「不適用」的格子用灰色標記
...
- 心理學驅動類型用獨立的標籤呈現（認知修正/價值連結/情感釋放/社會比較）
- 這是讀者打開報告後第一個看到的內容區塊（Hero 之後），要讓人在 30 秒內抓到全貌
- 摘要下方再接「最值得解決的問題」「最佳切入場景」「建議的第一步行動」「需要驗證的假設」等詳細分析卡片

**切入點詳細分析（可讀性原則）：**
- 每個分析項目不能只寫一句結論，必須附上脈絡說明——「為什麼選這個」「根據什麼判斷」
- 例如「最值得解決的問題」要列出 2-3 個選擇理由（覆蓋面、競品空白、可行性等）
- 「最佳切入點」要逐一說明每個功能為什麼被選中、預期效果是什麼
- 「需要驗證的假設」每條都要附上具體的驗證方式
- 整體原則：讀者看完這段後，能向別人解釋「為什麼是這個方向」，而不只是「方向是什麼」
...
**機會評估表（可讀性原則）：**
- HMW 欄位必須同時呈現「代號」和「短標題」（例如「HMW1 / 前 30 秒吸引力」），不能只放代號。建議用雙行呈現：代號粗體在上，短標題小字在下
- 「影響 Persona」欄位必須寫 Persona 全名（如「焦慮替代者、疲憊陪伴者」），不能只寫 P1、P2 等代號。讀者不應該需要翻回前面才知道 P1 是誰
- 表格下方的策略說明也要用全名或短標題，不能只用代號
